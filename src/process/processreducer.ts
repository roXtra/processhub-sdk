...
export function processReducer(processState: Process.ProcessState, action: any): Process.ProcessState {
  ...
  switch (action.type) {
    ...
    case PROCESSLOADED_MESSAGE: {
      processState.currentProcess = StateHandler.mergeProcessToCache((action as IProcessLoadedMessage).processDetails, StateHandler.rootStore.getState().processState, StateHandler.rootStore.getState().instanceState);
