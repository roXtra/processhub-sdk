...
import { Language, supportedLanguages } from "../tl.js";
import { sleep, getFormattedTimeZoneOffset, getFormattedDate, getFormattedDateTime } from "./timing.js";
    ...
    describe("getFormattedDateTime", function () {
      it("should format date time for all supported languages", () => {
        ...
        const localeTranslation: { [locale in Language]: string } = {
          ...
          "da-DK": "27.12.2021, 12.00",
          "sk-SK": "27. 12. 2021 12:00",
          "sl-SI": "27. 12. 2021, 12:00",
        };
        ...
        for (const locale of supportedLanguages) {
          ...
          // ICU versions can use typographic spaces without changing the displayed date or time.
          const formattedDateTime = getFormattedDateTime(now, locale).replace(/[\u00a0\u2009\u202f]/g, " ");
          expect(formattedDateTime, "Unexpected getFormattedDateTime output for " + locale).to.equal(localeTranslation[locale]);
        }
      });
    });
